...
const UpdateProduct = () => {
    ...
    if (title && price > 0 && images.every((url) => url !== "")) {
      try {
        const updateProduct = {
         ...
         await axios.put(`https://api.escuelajs.co/api/v1/products/${id}`, updateProduct)
  ...
  return (
    <div className="w-[320px] mx-auto mt-10 flex flex-col justify-start">
      <h1 className="text-4xl">Изменить продукт</h1>
      <input
        type="text"
      ...
      <div>
        ...
        <button className="w-full text-left underline" onClick={handleAddImage}>
          Добавить картинку
        </button>
      </div>
      <button
